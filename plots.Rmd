...
p_known <- c(
  "ABCA1", "SORT1", "LPA", "TNKS", "FASD3", "LIPC", "KPNB1", "PLTP", "APOC2"
)
JTI_known <- c(
  "SORT1", "LPA", "TNKS", "FASD3", "LIPC", "KPNB1", "PLTP", "APOC2", "ANGPTL3",
...
manhattan_ldl <- manhattan_base(ldl) +
  geom_point(aes(color = paste(ifelse(known, "Known", "Additional"), method))) +
  geom_label_repel(
    data = ldl %>% filter(known), 
    aes(label = gene_name, color = paste(ifelse(known, "Known", "Additional"), method)), 
    size = 3, alpha = 0.9, show_guide=FALSE
  ) +
